...
**Telemetry (`src/telemetry.ts`, doc in `TELEMETRY.md`).** Volume/health is counted in the `app.server.response` **metric** (`enableMetrics`), not off raw spans, so `beforeSendTransaction` can drop scanner routes and handshake-only notifications, and sample MCP `server/discover`/`ping`/`tools/list`/healthcheck `initialize` down to 1% without losing dashboards. Client attribution is a bounded `app.client.family` bucketed from the User-Agent (`resolveClientFamily`) and stamped on the root span — *not* the caller-controlled `mcp.client.name`, which is only a secondary trace attribute. `src/telemetry.ts` is pure (Node tsconfig, unit-tested); worker.ts owns the Sentry wiring.
...
| `MCP_ALLOWED_HOSTNAMES` (var, required) | Comma-separated hostname allowlist for MCP Host-header validation. The checked-in deployment allows `plausible-mcp.sentry.dev` plus localhost for `wrangler dev`; self-hosters set their own hostname. |
| `MCP_ALLOWED_ORIGIN_HOSTNAMES` (var, optional) | Comma-separated browser Origin hostnames allowed to call `/internal`; a present Origin is rejected when unset. Non-browser clients send no Origin. |
